---
name: code-review
description: "Review pull requests and diffs in langforge — the shared Go library for language and ecosystem detection — against the rios0rios0/guide standards, with extra weight on the hexagonal port set, provider registration, and version-file round-tripping. Use when reviewing a PR, a branch, or staged changes here."
---

# Code review — `langforge`

`langforge` detects languages and ecosystems (Go, Node, Python, Dart/Flutter, Java Gradle and Maven, C#, Ruby, Terraform, Dockerfile, pipelines) and reads and writes their version and dependency manifests. `autobump` and `autoupdate` rely on it to edit other people's repositories correctly.
...
## What matters most in `langforge`

These are the checks that catch real defects in this repository. Work through
them before the generic ones.

```
pkg/domain/entities/          Language, Version, Dependency, FileChecker, Classifier
pkg/domain/repositories/      ports: LanguageDetector, VersionReader, VersionWriter,
                              DependencyReader, DependencyUpdater, BuildValidator,
                              RuntimeManager, and the LanguageProvider composites
pkg/infrastructure/languages/ one adapter package per language (golang, node, python, dart,
                              javagradle, javamaven, csharp, ruby, terraform, dockerfile,
                              pipeline) — `java` is a shared JDK runtime manager, not a provider
pkg/infrastructure/registry/  LanguageRegistry: Language → LanguageProvider
pkg/infrastructure/versions/  latest-stable fetchers over public APIs
pkg/support/                  cmdexec, fileutil
```

- **A new language provider implements the existing ports and is registered in `LanguageRegistry`** — it never adds a `switch` on language. Detection-only ecosystems (`dockerfile`, `pipeline`) implement only what they need; the composites (`LanguageProvider`, `…WithValidation`, `…Full`) exist so a provider is not forced to stub methods it cannot support.
- **Version writing must round-trip.** A `VersionWriter` change has to preserve formatting, comments, and line endings in the manifest it edits — `autobump` commits the result to someone else's repository, and a reformatted `go.mod` or `package.json` produces an unreviewable diff.
- **`java` is not a provider.** It is the shared JDK runtime manager for `javagradle` and `javamaven`; the `golang` package is named `golang`, not `go`. Renaming either breaks imports downstream.
- **Dart covers Flutter** — both share `pubspec.yaml` and `IsFlutter` picks the toolchain. A change to detection must keep both paths working.
- **Version fetchers call public APIs.** They need timeouts, bounded responses, and a sane failure mode — a detection routine that hangs blocks every consumer.
- **Keep interfaces small.** Each port in `pkg/domain/repositories/` is one responsibility; widening one forces every provider to change.
- **This is a library — exported symbols are a contract** with `autobump` and `autoupdate`. Signature changes need the three-place breaking-change flag.
- Prefer returning `error` over panicking; `Must*` constructors belong in tests or package-level `var`/`init` blocks only. All public types and functions carry doc comments.

### Commands a reviewer should be able to quote

```bash
make setup && make lint && make test && make sast
go test -tags=unit ./...
go test -tags=unit -v ./pkg/infrastructure/languages/golang
go test -tags=unit -run TestGoVersionReader ./...
```
...
- `//go:build unit` on all unit tests, external `_test` packages (`package golang_test`).
- `t.Parallel()` at test-function and subtest level.
- Entities and stubs are constructed with the builders in `test/builders/` (`DependencyBuilder`, `VersionBuilder`, `LanguageProviderStubBuilder`); doubles live in `test/doubles/`. Framework: `testify` plus [`testkit`](https://github.com/rios0rios0/testkit).
- A manifest-editing change needs a fixture that includes comments and unusual formatting, asserting the untouched parts stay byte-identical.
...
- The per-language packages looking repetitive — that is the port contract.
