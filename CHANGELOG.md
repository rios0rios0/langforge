...
- added `repositories.MergeFilesChanged` and `cmdexec.CapturedVersion`, and moved every provider onto them. All nine `Provider.FilesChanged` methods carried a byte-identical copy of the same merge, and seven `RuntimeManager.CurrentVersion` methods differed only in a binary name and a regex — so the new Dart provider could not be written without either duplicating them again or extracting them. `csharp` and `node` keep their own `CurrentVersion`: those genuinely differ, trimming output and stripping a `v` prefix rather than matching a pattern
